...
export default class Gestures extends Component {
  ...
  onDrag = (event, gestureState) => {
    console.tron.log('onDrag')
    const { initialStyles } = this;
    const { draggable } = this.props;

    if (draggable) {
      console.tron.log('onDrag2')

      let left = initialStyles.left;
      if(_.isString(initialStyles.left) && initialStyles.left.endsWith('%'))
      {
        const percentChange = gestureState.dx / this.state.size.width * 100;
        const oldStylePct = parseFloat(initialStyles.left)
        left = (percentChange + oldStylePct) + "%"
      }
      else {
      ...
      {
        const percentChange = gestureState.dy / this.state.size.height * 100;
        const oldStylePct = parseFloat(initialStyles.top)
        top = (percentChange + oldStylePct) + "%"
      }
      else {
